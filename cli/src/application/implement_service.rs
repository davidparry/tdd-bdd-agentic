...
use crate::application::assets::{asset_survey, find_requirement};
use crate::application::generation_service::ResolvedLlm;
use crate::application::spec_service::ServiceError;
use crate::domain::generation::{
    FileUpdate, ImplementAsset, advice_prompt, implementation_prompt, implementation_target_path,
    parse_file_updates, strip_code_fences,
};
    ...
    L: LlmGenerator,
{
    ...
    pub fn generate(
        ...
        let files: Vec<(String, String)> = sources
            ...
            .collect();
        let production = implementation_target_path(self.language, &spec.project);
    ...
    pub fn advice(
        ...
        tracing::debug!(requirement = %req_id, purpose = "implement advice", "calling LLM");
        let reply = llm
            .generator
            .generate(&llm.model, &prompt.system, &prompt.user)
            .map_err(|e| ServiceError(format!("the model call failed - {}", e.0)))?;
