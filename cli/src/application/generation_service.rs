...
use crate::application::assets::{find_missing_steps, find_requirement};
use crate::application::spec_service::ServiceError;
use crate::domain::generation::{
    looks_like_step_definitions, looks_like_unit_test, polish_prompt, step_definitions_template,
    steps_target_path, strip_code_fences, unit_test_target_path, unit_test_template,
};
    ...
    L: LlmGenerator,
{
    ...
    /// criteria (unit_test_create).
    pub fn unittest_generate(&self, req_id: &str) -> Result<GenerationReport, ServiceError> {
        let spec = self.spec.load().map_err(|e| ServiceError(e.0))?;
        let requirement = find_requirement(&spec, req_id)?;
        let template = unit_test_template(self.language, requirement);
        let (content, source) =
            self.polish(&template, |code| looks_like_unit_test(self.language, code));
        let target = unit_test_target_path(self.language, req_id);
        let summary = format!(
    ...
    fn polish(&self, template: &str, valid: impl Fn(&str) -> bool) -> (String, String) {
        ...
        match llm
            .generator
            .generate(&llm.model, &prompt.system, &prompt.user)
        {
            Ok(response) => {
...
mod tests {
    ...
    fn generate_without_a_model_stages_the_template() {
        ...
        assert_eq!(report.target, "src/test/java/steps/GeneratedSteps.java");
        assert!(report.staged);
        let changes = staged(&service);
        assert_eq!(changes.len(), 1);
        assert_eq!(changes[0].path, "src/test/java/steps/GeneratedSteps.java");
