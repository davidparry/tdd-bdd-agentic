    ...
    SpecMutationService<R, F, G, C, S>
{
    ...
    pub fn draft_assisted_in(
        ...
        let reply = generate_logged(llm, model, &prompt);
        drop(work);
        let proposals = match reply {
    ...
    pub fn set_feature(&self, id: &str, path: &str) -> Result<SetFeatureReport, ServiceError> {
        ...
        self.stage_file(&catalog, &target, &format!("set {id} featureFile to {path}"))?;
        Ok(SetFeatureReport {
            id: id.to_string(),
            feature_file: path.to_string(),
            staged: true,
    ...
    pub fn include_add(&self, path: &str, from: Option<&str>) -> Result<IncludeReport, ServiceError> {
        let mut catalog = self.effective_catalog()?;
    ...
    fn rewording(
        ...
        for (index, finding) in findings.iter().enumerate() {
            ...
            let outcome = generate_logged(llm, model, &prompt);
            drop(work);
...
mod tests {
    ...
    fn a_draft_into_an_included_file_stages_only_that_file() {
        ...
        let child: Spec =
            serde_json::from_str(&service.store.content("requirements/core.json").unwrap().unwrap())
                .unwrap();
    ...
    fn listed_requirements_name_the_file_they_live_in() {
        ...
        assert!(
            listed
                .iter()
                .all(|r| r.file == "requirements/core.json"),
            "listed: {listed:?}"
        );
    }
    ...
    fn include_add_of_an_already_included_file_is_a_no_op() {
        ...
        assert!(!report.staged);
        assert!(!report.created);
        assert!(report.next_step.contains("already part of the spec catalog"));
    }
