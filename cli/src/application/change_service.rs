...
impl<C: ChangeStore, R: SpecRepository, F: FeatureFiles> ChangeService<C, R, F> {
    ...
    pub fn validate(&self) -> Result<ValidationReport, ServiceError> {
        ...
        match self.effective_spec()? {
            Ok(spec) => {
                let overlay = OverlayFeatures {
                    store: &self.store,
                    fallback: &self.feature_files,
                };
                issues.extend(SpecValidator::new(&overlay).validate(&spec));
            }
            Err(spec_issue) => issues.push(spec_issue),
        }
    ...
    /// The spec as it would look after commit. A malformed staged spec is
    /// a validation issue (inner `Err`), not a hard failure.
    fn effective_spec(&self) -> Result<Result<Spec, String>, ServiceError> {
        match self
            .store
            .content(&self.spec_path)
            .map_err(|e| ServiceError(e.0))?
        {
            Some(text) => Ok(serde_json::from_str(&text).map_err(|e| {
                format!("spec: staged {} is not readable JSON - {e}", self.spec_path)
            })),
            None => Ok(self.repository.load().map_err(|e| e.0)),
        }
    }
}
