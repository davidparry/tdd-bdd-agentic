    ...
    fn get_info(&self) -> ServerInfo {
        ...
        info.instructions = Some(
            "Drives a spec-driven TDD/BDD workflow. The requirements spec is the source \
             ...
             failing unit test -> run_tests (expect RED) -> implement -> run_tests \
             (expect GREEN) -> start_refactor -> run_tests. The human developer stays \
             in control of every engineering decision."
                .to_string(),
        );
        info
    }
