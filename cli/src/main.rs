...
/// Diagnostics go to stderr so stdout stays clean for JSON output and the
/// MCP stdio protocol. `RUST_LOG` overrides both the default and `--debug`.
...
fn generation_service(
    ...
    GenerationService<GherkinFeatureCatalog, FsSourceFiles, FsChangeStore, FsSpecRepository, Llm>,
> {
...
fn implement_service(
    ...
    ImplementService<GherkinFeatureCatalog, FsSourceFiles, FsChangeStore, FsSpecRepository, Llm>,
> {
    let language = primary_language(root)?;
    let llm = resolved_ollama(root, model_flag)
        .map(|(model, generator)| ResolvedLlm { model, generator });
...
fn status_service(
    ...
    StatusService<GherkinFeatureCatalog, FsSourceFiles, FsChangeStore, FsSpecRepository, Llm>,
> {
    let llm = resolved_ollama(root, model_flag)
        .map(|(model, generator)| ResolvedLlm { model, generator });
