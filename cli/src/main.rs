...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Some(Command::Greenfield) => {
            match execute(&cli.root, cli.model.as_deref(), &Command::Greenfield) {
                Err(error) if error.is::<NonzeroExit>() => {
                    drop(log_guard);
                    std::process::exit(1)
                }
                Err(error) => Err(error),
                Ok(()) => resume_shell_after_greenfield(&cli.root, cli.model.as_deref()),
            }
        }
...
fn init_logging(debug: bool, root: &Path) -> Option<tracing_appender::non_blocking::WorkerGuard> {
    ...
    let default_directives = if debug {
        "bdd=debug,bdd_cli=debug"
    } else {
        "bdd=info,bdd_cli=info"
    };
    let filter =
...
fn run_shell_mode(root: &Path, model: Option<&str>) -> anyhow::Result<()> {
    ...
    interactive_shell_loop(root, model, true, model_ready)
}
...
fn resume_shell_after_greenfield(root: &Path, model: Option<&str>) -> anyhow::Result<()> {
    use std::io::IsTerminal as _;
    if !std::io::stdin().is_terminal() {
        return Ok(());
    }
    ...
    interactive_shell_loop(root, model, false, false)
}
    ...
    let report = match command {
        ScenarioCommand::Add {
            ...
            let report = service
                ...
                .map_err(|e| anyhow::anyhow!(e.0))?;
            let _ = mutation_service(root).set_feature(req, feature);
            report
        }
...
fn run_spec(root: &Path, model: Option<&str>, command: &SpecCommand) -> anyhow::Result<()> {
    ...
    match command {
        SpecCommand::List => {
            let requirements = mutation_service(root)
                .list_requirements()
                .map_err(|e| anyhow::anyhow!(e.0))?;
        ...
        SpecCommand::Draft {
            ...
            let mutations = mutation_service(root);
            if title.is_some() || story.is_some() || !criterion.is_empty() {
                ...
                let report = mutations
                    .draft_direct(title, story, criterion.clone())
                    .map_err(|e| anyhow::anyhow!(e.0))?;
            ...
            let report = match resolved_ollama(root, model) {
                Some((name, generator)) => {
                    mutations.draft_assisted(prompter.as_mut(), &name, &generator)
                }
                None => mutations.draft(prompter.as_mut()),
            }
            .map_err(|e| anyhow::anyhow!(e.0))?;
            print_json(&report)
        }
...
type Llm = CachedGenerator<OllamaGenerator>;
