...
walks students through the loop manually against the Java
`tdd-workflow-server`; this binary automates the same loop with
byte-identical tool replies, so the lesson and the tool never drift
apart.
...
cannot be served. Run with `--debug` to trace cache hits and misses on
stderr.

## Debug logging

Every command accepts a global `--debug` flag that turns on verbose
diagnostic logging on stderr: the full prompts sent to the model and
its responses, cache hits and misses, resolved configuration, MCP tool
calls, and test-runner activity. Without the flag only high-level
lifecycle events are logged. Logs never touch stdout, so JSON output
and the MCP stdio protocol stay clean either way.

```bash
bdd --debug implement REQ-003        # watch the prompt and the reply
bdd --debug spec list 2> debug.log   # capture diagnostics separately
```
