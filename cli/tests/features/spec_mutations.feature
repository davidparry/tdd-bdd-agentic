...
Feature: Spec mutations
  ...
  Scenario: A described feature is split into proposals the wizard walks through
    ...
    And the developer was told a finding containing "Left for later runs: Comma separated numbers are summed."
    And the developer was asked "REQ-002 title [Empty string returns zero] (Enter keeps it):"
    And the developer was asked "REQ-002 criterion 1 [Given an empty string "", when add is called, then the result is 0] (Enter keeps it, '-' drops it):"
    And the staged spec has 2 requirements
