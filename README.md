...
| [`cli/`](cli/README.md) | The `bdd` CLI (Rust): one native binary that automates the same spec-driven loop the workshop teaches — spec validation and wording refinement, Ollama model selection, project/runtime inspection across Java, JavaScript/TypeScript, .NET, and Rust — with an embedded MCP server keeping the seven tool contracts above, byte-identical. Clean architecture, 100% test coverage, its own spec-driven Cucumber suite. See [`cli/README.md`](cli/README.md) and the searchable [command manual](https://davidparry.github.io/tdd-bdd-agentic/manual/). |
